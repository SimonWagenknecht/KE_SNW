...
//				Modb_Rx_Ready(port);	nach Timer-IT verlagert
				timer_function = 1;							// Für Timer-ISR: RTS-Delay aktivieren
  ...
  switch(modb_rx_int_state)
	{
		...
		case 1:
			if(modb_rx_count < modb_rx_buff_size)			// Länge Testen
			{	
...
void U_Modbus_ISR_Tel_Tout(char port)
{
	...
	if(port == S1)
	{
		...
		{
			if((pRxBuf[0] == modb_curr_adr) && (modb_rx_count > 3))	// aus Zeitgründen nur gültige Frames dem Handler melden
			...
			{ 			
				...
				timer_function = 0;							// Für Timer-ISR: EoF für Rx-Funktion einschalten
			}
		}
	}
	#endif
...
void start_modb_transmit(char port)
{
	...
	// Frame Check Sequenz berechnen
		...
		modb_leng_tx = modb_tx_count;		// Telegrammlänge	für	Sendeinterrupt
...
void set_EoF_timer(char port)
{
	timer_function = 0;						// Für Timer-ISR: EoF-Funktion
	switch(port)
	{
		case S1:
			P_TOUTS_S1	= 0;					// Stop	 Prescaler
...
//			save_tx_count = modb_tx_count;					// für Response-Check
			break;
